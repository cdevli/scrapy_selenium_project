...
class JdSpiderSpider(CrawlSpider):
    ...
    rules = [
        Rule(LinkExtractor(allow=()),  # 1，允许抓取所有链接
    ...
    def parse_shop(self, response):
        pass
        ware_id_list = list()
        url_group_shop = LinkExtractor(allow=(r'(http|https)://item.m.jd.com/product/\d+.html')).extract_links(
            response)  # 2，对链接使用正则表达式进行筛选

        re_get_id = re.compile(r'(http|https)://item.m.jd.com/product/(\d+).html')  # 3，定义抓取正则表达式的抓取规则
